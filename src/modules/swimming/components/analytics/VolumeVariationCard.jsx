...
export function VolumeVariationCard({ workouts, windowWeeks }) {
  ...
  const data = useMemo(
    () => vol.weeks.map((w, i) => ({ label: w.label, distanceM: w.distanceM, variacao: vol.weekOverWeek[i] })),
    [vol]
  );
  ...
  return (
    <Card>
      <CardHeader
        ...
        description={`Últimas ${windowWeeks} semanas · média ${fmtDistanceM(vol.avgDistanceM)} · variação (CV) ${
      ...
      <div style={{ height: 240 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ left: -10, right: 8 }}>
            ...
            <YAxis yAxisId="dist" tick={{ fill: C.gray, fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis
            ...
            <Tooltip
              ...
              formatter={(v, name) => (name === "distanceM" ? [fmtDistanceM(v), "Volume"] : [v !== null ? `${v.toFixed(0)}%` : "—", "Variação"])}
            />
            <Bar yAxisId="dist" dataKey="distanceM" radius={[6, 6, 0, 0]} fill={natacao.color} />
