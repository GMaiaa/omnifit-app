...
export function InsightsPanel({ workouts, paceWindowWeeks }) {
  const insights = useMemo(
    () => generateInsights(workouts, { paceWindowWeeks }),
    [workouts, paceWindowWeeks]
  );
    ...
    return (
      <Card>
        <p className="text-sm" style={{ color: C.gray }}>
          ...
          sobre sua evolução na água aparecem aqui.
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {insights.map((insight) => {
        ...
        const accent = isWarning ? C.amber : natacao.color;
        return (
          <div
            key={insight.id}
