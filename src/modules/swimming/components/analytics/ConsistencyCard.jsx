...
function heatColor(count) {
  ...
  if (count === 1) return `${natacao.color}55`;
  if (count === 2) return `${natacao.color}99`;
  return natacao.color;
}
...
function MiniStat({ icon: Icon, label, value }) {
  return (
    <div className="flex items-center gap-2.5 rounded-xl px-3 py-2.5" style={{ background: C.surface2, border: `1px solid ${C.borderSoft}` }}>
      <div className="rounded-lg p-1.5" style={{ background: `${natacao.color}1A`, color: natacao.color }}>
        <Icon size={14} />
      </div>
      <div>
