...
import { fmtDateShort, fmtElevation, fmtSpeed } from "../../../../lib/format";
...
export function RecordsCard({ workouts, typeId }) {
  ...
  const data = useMemo(
    () => series.map((p) => ({ label: fmtDateShort(p.date), speedKmh: p.speedKmh, isPR: p.isPR })),
    [series]
  );
  ...
  return (
    <Card>
      <CardHeader
        ...
        description="Melhor velocidade por tipo de treino e distância"
      />
                ...
                {DISTANCE_BUCKETS.map((b) => {
                  ...
                  return (
                    <td key={b.id} className="text-right px-2 py-1.5" style={{ color: rec ? C.white : C.gray }}>
                      {rec ? `${fmtSpeed(rec.speedKmh)} km/h` : "—"}
                    </td>
                  );
                })}
              </tr>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ left: -20, right: 8 }}>
              ...
              <Tooltip
                ...
                formatter={(v) => [`${fmtSpeed(v)} km/h`, "Velocidade"]}
              />
              <Line
                type="monotone" dataKey="speedKmh" stroke={type.color} strokeWidth={2.5}
                dot={(props) => {
      ...
      {(records.longestRide || records.bestWeek || records.bestClimb) && (
        <div className="flex gap-4 mt-3 pt-3 flex-wrap" style={{ borderTop: `1px solid ${C.borderSoft}` }}>
          {records.longestRide && (
            <div className="flex items-center gap-2 text-xs" style={{ color: C.gray }}>
              ...
              Maior distância: <span style={{ color: C.white, fontWeight: 600 }}>{records.longestRide.distanceKm.toLocaleString("pt-BR")} km</span>
            </div>
          )}
