...
export function InsightsPanel({ workouts, speedWindowWeeks }) {
  const insights = useMemo(
    () => generateInsights(workouts, { speedWindowWeeks }),
    [workouts, speedWindowWeeks]
  );
    ...
    return (
      <Card>
        <p className="text-sm" style={{ color: C.gray }}>
          ...
          sobre sua evolução aparecem aqui.
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {insights.map((insight) => {
        ...
        const accent = isWarning ? C.amber : ciclismo.color;
        return (
          <div
            key={insight.id}
